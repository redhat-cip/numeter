...
class Host_GET_detail_Test(APILiveServerTestCase):
    ...
    def test_foreign_host(self):
        ...
        self.DETAIL_URL = reverse('host-detail', args=[Host.objects.exclude(pk=self.host.id)[0].id])
        r = self.user_client.get(self.DETAIL_URL)
...
class Host_DELETE_list_Test(APILiveServerTestCase):
    """
    Test DELETE list. Same as
    ``curl -i -X DELETE http://127.0.0.1:8081/rest/hosts/ -H 'Accept: application/json'``
    """
    @set_storage(extras=['host'])
    @set_users()
    @set_clients()
    ...
    def test_simple_user(self):
        """Forbidden access to simple user."""
        r = self.user_client.delete(LIST_URL)
        self.assertEqual(r.status_code, 403, 'Bad response (%i)' % r.status_code)
