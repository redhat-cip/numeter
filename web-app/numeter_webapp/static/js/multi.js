...
(function ($, numeter) {
  ...
  $(document).on('submit', '#main-q', function(e) {
    ...
    $(into).empty();
    numeter.print_loading_gif(into, '25%', '25%')
  ...
  $(document).on('click', '#resolution-pills li a', function() {
    ...
    $.each(numeter.graphs, function(i,g) {
      numeter.update_graph(g, res);
    });
  });
  ...
  $(document).on('click', '#toggle-editor', function() {
    if (! $(this).parent().hasClass('active') ) {
      ...
      numeter.print_loading_gif('#multiview-index', '200px', '200px');
  ...
  $(document).on('submit', ".view-form", function() {
    ...
    $.ajax({type:method, url:url, async:true,
      data: $(form).serialize(),
      error: function(data, status, xhr) { numeter.error_modal(data) },
      success: function(data, status, xhr) {
        $('.messages').append(data['html']);
        // IF FORM IS VALIDATE
        if ( data['response'] == 'ok' ) {
  ...
  $(document).on('click', ".edit-multiview", function() {
    ...
    numeter.print_loading_gif(into, 60, 60);
  ...
  $(document).on('submit', ".event-form", function(e) {
    ...
    $.ajax({type:method, url:url, async:true,
      ...
      success: function(data, status, xhr) {
        ...
        // IF FORM IS VALIDATE
        if ( data['response'] == 'ok' ) {          
          var url = data['callback-url'];
          $(into).empty();
          ///multiviews/customize/event/1
          numeter.print_loading_gif(into, '25%', '25%');
          $.ajax({type:'GET', url:url, async:true,
            error: function(data, status, xhr) { numeter.error_modal() },
            success: function(data, status, xhr) {
